...
class Conection:
    ...
    def __connect(self):
        ...
        if self.Msg == "":
            if (
                self.cnn_name == "DB_CONFIG_DWH"
                or self.cnn_name == "DB_CONFIG_TEST"
                or self.cnn_name == "DB_CONFIG_ESTADO"
            ):
                try:
            ...
            self.Msg = (
                "No se han especificado los siguientes parametros:\n {0}",
                format(self.Msg),
            )
            self.cnn = None
            return False
        ...
        if self.Script != "":
            ...
            if self.__connect():
                try:
                    ...
                    self.DFData = pd.DataFrame.from_records(
    ...
    def insertFile(self, oppID, actaID, NameFile, ext, url):
        if self.__connect():
            try:
                cursor = self.cnn.cursor()
                cursor.execute(
                    "INSERT DWH_Analytics.dbo.CNet_ActasJuntaAclaraciones ([opportunityID],[actaID],[nombre_Archivo],[extension],[url_Descarga]) OUTPUT INSERTED.opportunityID VALUES ({0},{1}, '{2}','{3}','{4}')".format(
    ...
    def update_OppID(self, oppID, ActaDescargada=0, ResultadoConsulta=""):
        if self.__connect():
            try:
                cursor = self.cnn.cursor()
                cursor.execute(
                    """Update DWH_Analytics.dbo.CNet_Opportunities 
                    set [FechaUltimaConsulta]=getdate(),[ActaDescargada]={0}, 
                    [resultadoConsultaDesc]='{1}' 
                    where opportunityId={2} """.format(
                        ActaDescargada, ResultadoConsulta, oppID
                    )
                )
                self.cnn.commit()
    ...
    def GetIdEstados(self):
        query_id_estados = """
        SELECT 
        CASE WHEN es.Estado = 'ESTADO DE MEXICO' THEN 'MEXICO'
        ELSE es.Estado END as Estado ,
    ...
    def InsertData(self, DF_Data, TableName, FieldList=[]):
        ...
        scriptText = (
            "INSERT INTO dbo." + TableName + " (" + cols + ") VALUES (" + params + ")"
        )
        self.__connect()
        cursor = self.cnn.cursor()
    ...
    def getQuery(self, query: str, campos: list = []):
        """Método para obtener datos a partir de un query
        ...
        if len(campos) > 0:
            results = pd.DataFrame(cursor.fetchall(), columns=campos)
        else:
            cols = [str(i[0]) for i in cursor.description]
