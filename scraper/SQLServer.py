...
class SQLConection:
    def __init__(self):
        ...
        self.SecurityIntegrated_edo = config["DB_ESTADOCONFIG"][
            "DB_SECURITY_INTEGRATED"
        ]
    ...
    def SelectFromFile(self, PathScriptFile):
        ...
        if (
            os.path.exists(self.PathScriptFile)
            and os.path.getsize(self.PathScriptFile) > 0
        ):
    ...
    def ExportToExcel(
        self, PathFileExcel, SheetName="Data", TableName="TData", CloseFileExcel=True
    ):
        self.Msg = ""
        if self.Rows > 0:
