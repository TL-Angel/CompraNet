...
class SQLConection:   
    def __init__(self):   
        ...
        with open('ODBC/config.json', 'r') as file:
    ...
    def __connect(self):        
        self.Msg = ""
        if (self.Driver == ""): self.Msg = self.Msg + "- Driver.\n"
        ...
        if (self.Msg == ""):
            try:
                ...
                if self.SecurityIntegrated.lower() == "no":
                    self.cnn = pyodbc.connect('Driver=' + self.Driver + ';Server=' + self.Server + ';Database=' + self.Database + ';UID=' + self.UID + ';PWD=' + self.PWD + ';APP=ROBINA;')
                else:
                    self.cnn = pyodbc.connect('Driver=' + self.Driver + ';Server=' + self.Server + ';Database=' + self.Database + ';Trusted_Connection = ' + self.SecurityIntegrated + ';')            
                
            except pyodbc.Error as e:                
                self.Msg="Err Number : {0} \nErr description: {1}".format(e.args[0],e.args[1])
                self.cnn = None
                return False  
        ...
        if (len(DF_Data)>=1 and TableName!=''):
            if self.__connect():
                try:
                    ...
                    scriptText="INSERT INTO dbo."+ TableName +" ("+ cols +") VALUES ("+ params +")"              
                    cursor=self.cnn.cursor()
                    ...
                    self.cnn.commit()
                    cursor.close()                    
                    self.Msg="Se insertaron los registros correctamente en la tabla:{0}".format(TableName)                  
                finally:                
                    self.__disconect()
        else:
            self.Msg="Datos insuficientes para agregar registros [Registros en DF:{0}, Nombre de la tabla:{1}].".format(len(DF_Data),TableName)
    ...
    def ExportToExcel(self, PathFileExcel, SheetName="Data", TableName="TData", CloseFileExcel=True):
        ...
        if (self.Rows > 0):
            ...
            self.DataRange = xlsxwriter.utility.xl_range(0, 0, self.Rows, len(self.Headers)-1)

            # Escribimos los datos en el archivo de Excel
            ws.add_table(self.DataRange, {'name': self.TableName, 'data': self.Data, 'columns': self.HeadersToExcel})
